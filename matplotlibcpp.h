...
PyObject* get_array(const std::vector<Numeric>& v)
{
    ...
    void plot_surface_with_vector_field(const std::vector<::std::vector<Numeric>> &x,
                                        const std::vector<::std::vector<Numeric>> &y,
                                        const std::vector<::std::vector<Numeric>> &z,
                                        const std::vector<Numeric> &x2,
                                        const std::vector<Numeric> &y2,
                                        const std::vector<Numeric> &z2,
                                        const std::vector<Numeric> &i,
                                        const std::vector<Numeric> &j,
                                        const std::vector<Numeric> &k,
                                        double quiverLength = 0.1,
                                        const std::map<std::string, std::string> &keywords =
                                        std::map<std::string, std::string>())
    {
        // We lazily load the modules here the first time this function is called
        ...
        assert(y2.size() == z2.size());
        assert(z2.size() == i.size());
        ...
        PyObject *x2array = get_array(x2);
        PyObject *y2array = get_array(y2);
        PyObject *z2array = get_array(z2);

        PyObject *iarray = get_array(i);
        PyObject *jarray = get_array(j);
        PyObject *karray = get_array(k);
        ...
        // args and kwargs for quiver call
        PyObject *args2 = PyTuple_New(6);
        PyTuple_SetItem(args2, 0, x2array);
        PyTuple_SetItem(args2, 1, y2array);
        PyTuple_SetItem(args2, 2, z2array);
        PyTuple_SetItem(args2, 3, iarray);
        PyTuple_SetItem(args2, 4, jarray);
        ...
        Py_INCREF(quiver);
        PyObject *res2 = PyObject_Call(quiver, args2, kwargs2);
        if (!res2) throw std::runtime_error("failed quiver");
        Py_DECREF(quiver);
