...
//     }
//     double len = sqrt((line[3])*(line[3]) + (line[4])*(line[4]) + (line[5])*(line[5]));
//     // // [;t::plot_surface_with_line_and_axes_set(xs,ys,zs,axes,{line[0]},{line[1]},{line[2]},{line[3]},{line[4]},{line[5]},len);
// }
...
__global__
void orthogonalProjection(Model *m, double *pointArr, double *dists, double u, double v, double eps = 0.0000001, int iter = 100){
    ...
    double P[3] = {pointArr[3*index + 0],pointArr[3*index + 1],pointArr[3*index + 2]};

    double p0[3],p0P[3],du[3],duu[3],duv[3],dvv[3],dv[3],n[3],coefs[3],Ps[3],q[3],c1[3],c2[3],cq[3],s[3];
...
void TestGeometricParallelized(Model model, int numPoints = 1000, int preprocess = 0, double eps = 0.0000001){
  int seed = 1;
  ...
  double *dists;
  Model *m;
  cudaMallocManaged(&Ps, numPoints*3*sizeof(double));
  cudaMallocManaged(&dists, numPoints*sizeof(double));
  cudaMallocManaged(&m, sizeof(Model));
  ...
  *m = model;
  ...
  if(blockSize > numPoints)
    orthogonalProjection<<<1, numPoints>>>(m, Ps, dists, start[0], start[1], eps);
  else
    orthogonalProjection<<<numBlocks, blockSize>>>(m, Ps, dists, start[0], start[1], eps);
  cudaDeviceSynchronize();

  auto t_stop = chrono::high_resolution_clock::now();
  auto t_duration2 = chrono::duration_cast<chrono::microseconds>(t_stop - t_start);

  cout << "GPU computation time: " << t_duration2.count() << " microseconds " << endl;
  cudaFree(Ps);
  cudaFree(dists);
  cudaFree(m);
}
